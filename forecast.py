...
def forecast(data, config, interval):
    name = f'Forecast_{data.datetime.min().strftime("%d %b %Y")} - ' + \
    ...
    for sentiment in sentiments:
        ...
        preds, conf_int = model.predict(n_periods=test.shape[0],
                                        return_conf_int=True)

        logging.info('Performance')
        print(
            f"   *  Test RMSE : {round(np.sqrt(mean_squared_error(test, preds)), 2)}"
        )
        print(
            f"   *  Test MAPE : {round(mean_absolute_percentage_error(test, preds) * 100, 2)}%"
        )
        print()

        model.update(test)

        preds, conf_int = model.predict(n_periods=config['n-predict'],
                                        return_conf_int=True)
        if interval == 'day':
            i = 'D'
        elif interval == 'hour':
        ...
        plt.xlabel("Date")
        plt.savefig(os.path.join(project_dir, f'ARIMA-{sentiment}.png'),
                    bbox_inches="tight")
...
if __name__ == '__main__':
    ...
    if args.analyze:
        ...
        forecast(data, config['forecast'],
                 config['data']['forecast']['groupby'])
